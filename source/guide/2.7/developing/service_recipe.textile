...
* *"Lifecycle Events":lifecycle_events.html*: Maps events to handling scripts or closures and, as such, is the most important part of the service recipe. An event handler can be an external Groovy script, a shell script, or a batch file (depending on the operating system). The handlers must reside in the service folder
...
* *"Custom Commands":custom_commands.html*: Maps custom commands to handling scripts that can be invoked as commands using the Cloudify shell
* *"Probes":plugins.html*: For monitoring service configuration, performance, and availability
* *"UI Layout":configuring_ui_section.html*: Describes the layout to use for displaying the collected metrics in the "Cloudify web management console":../monitoring/the_web_management_console.html
...
For example, you can create a standard layout for your "web management console":configuring_ui_section.html metrics, as follows:
