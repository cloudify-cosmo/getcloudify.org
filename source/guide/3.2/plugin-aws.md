...
The AWS plugin allows users to use an AWS based cloud infrastructure for deploying services and applications.
  ...
  * `parameters` key-value server configuration as described in [AWS EC2 Classic](http://boto.readthedocs.org/en/latest/ref/ec2.html#module-boto.ec2.instance).
    * **Notes:**
      * The public key which is set for the server needs to match the private key name in your AWS account. The public key may be set in a number of ways:
        * By connecting the server node to a keypair node using the `cloudify.aws.relationships.instance_connected_to_keypair` relationship.
...
Four additional runtime-properties are available on node instances of this type once the `cloudify.interfaces.lifecycle.start` operation succeeds:
...
Note that the actual IP is available via the `elastic_ip` runtime-property.



# Relationships

In general, relationships represent node dependenies within Cloudify.
 ...
 * `create` If use_external_resource is true, the AWS plugin will check if the resource is available in your account. If no such resource is available, the operation will fail, if it is available, it will assign the resource id to the instance runtime_properties.
...
* The plugin needs access do your aws_access_key_id and aws_secret_access_key in order to operate.
