...
* **Infrastructure** - Components provided by the IaaS layer (e.g. VM, virtual network, virtual load balancer or storage volume) or by non-cloud physical or virtualized layer (e.g. Host and storage volume)
...
*  Use concrete types with your blueprint to make them more easy to read. Use portable nodes ONLY if you plan to deploy the same blueprint on different clouds.
...
The `type implementation` which is specific for a particular environment or toolchain will have specific properties declared by the concrete type and specific values for this particular instance.
...
Relationship describes the type of dependency between 2 nodes. There are 3 types of abstract built-in relationships:
...
## Custom Workflows
In order to add a custom workflow you need to create a radial script in the same folder of your `blueprint`
