...
<li><a href="http://docs.getcloudify.org/3.4.0/plugins/openstack/#tips"><strong>NIC Ordering</strong></a> - Respecting relationship order in blueprints, enabling determining which network is assigned to which vNIC, including control over the management network.</li>
<li>Infrastructure + OpenStack plugin support, other plugins updates to follow.</li>
</ul>
...
<li><a href="http://docs.getcloudify.org/3.4.0/manager/maintenance-mode/"><strong>Manager Maintenance Mode</strong></a> - Allows the Cloudify administrator to halt running operations in a safe manner, and to prevent from user to trigger new executions, to maintain and upgrade the Cloudify manager.</li>
<li><strong>Thousands of Deployments Scalability</strong> - Overhaul of the manager workers architecture improves the scalability of the Cloudify Manager by removing the dedicated deployment workers and consolidating all task executions to be handled by the Cloudify Management Worker</li>
</ul>
...
<li>In addition to existing Cloudify Manager bootstrap methods, Cloudify now offers&nbsp; AWS <a href="http://getcloudify.org/downloads/get_cloudify.html">AMI images and QCOW Image</a> for OpenStack for simpler and quicker installation.</li>
</ul>
...
<p>The Cloudify Composer is a graphical editor for creating blueprint YAML files dynamically using a modern drag and drop interface. Cloudify Composer enables topology modeling for complex applications while providing means to add relevant lifecycle operations implementation via external plugins and scripts.</p>
<ul>
...
<span style="margin-left:5em">Rewrote API server</li></span><br>
<span style="margin-left:5em">Added service logging</li></span><br>
<span style="margin-left:5em">Changed backend to TinyDB</li></span><br>
<span style="margin-left:5em">Added mixed-OS pool support</li></span><br>
<span style="margin-left:5em">Removed requirement for config YAML file</li></span><br>
<span style="margin-left:5em">Added non-daemon service option</li></span><br>
<span style="margin-left:5em">Added flask-restful exception handling</li>
...
<span style="margin-left:5em">Changing node type.</span><br>
<span style="margin-left:5em">In both cases, in order to make those changes, renaming of the node is required. The old node would be deleted and a new one would be <span style="margin-left:5em">installed (This behaviour is consistent with those change - changing the contained in or modifying node type).</span></span>
</ul>
<ul>
