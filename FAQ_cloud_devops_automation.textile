 ...
 *A:* Cloudify is an open source framework that allows you to deploy, manage and scale your applications on the cloud, and we tried to make this as simple as possible. The idea is simple: you model your application using a blueprint, which describes the tiers of which it consists, and how to install, start and monitor each of these tiers. The blueprint is a collection of text files which describe all of the above. Each Cloudify deployment has one or more managers, which are used to deploy new applications (using the above blueprints), and continuously monitor, scale and heal existing applications. 
