...
I am still amazed from time to time to find that even very intelligent people who know a lot about software development, but nothing about cloud computing, fail to know that in the cloud "world" : when a machine (VM) goes down, its data is gone forever, unless you take care of that in advance.
...
Storing your data on a cloud, is much cheaper than in your offices, but regarless of the price, it will lead to a disaster if you don't define and implement automated backup and storage procedures.

Cloudify comes with "Storage API support":http://www.cloudifysource.org/guide/2.5/developing/storage which is currently only for the EC2 EBS and for "Openstack IaaS" storage.
...
I've enhanced the Cloudify "(newwindow)MySQL recipe":https://github.com/CloudifySource/cloudify-recipes/tree/master/services/mysql and it now enables users to install MySQL on any cloud and then create a snapshot (mysqldump) and store it on "(newwindow)Amazon S3":http://aws.amazon.com/s3.
...
The MySQL recipe itself is available  "(newwindow)here":https://github.com/CloudifySource/cloudify-recipes/tree/master/services/mysql 
