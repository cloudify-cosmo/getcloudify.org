...
* *"Lifecycle Events":../developing/lifecycle_events*: Maps events to handling scripts or closures and, as such, is the most important part of the service recipe. An event handler can be an external Groovy script, a shell script, or a batch file (depending on the operating system). The handlers must reside in the service folder
...
* *"Custom Commands":../developing/custom_commands*: Maps custom commands to handling scripts that can be invoked as commands using the Cloudify shell
