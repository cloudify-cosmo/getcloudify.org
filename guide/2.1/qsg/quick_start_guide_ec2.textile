...
<img src="/guide/2.1/images/ec2/EC2_CREATE_KEY_PAIRS_RAW3.jpg" />
...
<img src="/guide/2.1/images/ec2/EC2_CREATE_KEY_PAIRS_1_RAW3.jpg" />
...
<img src="/guide/2.1/images/ec2/EC2_CREATE_KEY_PAIR_SUCCESS_AND_SAVE3.jpg" />
...
<img src="/guide/2.1/images/ec2/EC2_ACCESS_KEY_AND_SECRET_KEY3.jpg" />
...
In our example, the PetClinic application has 5 services:
...
* Apache Load Balancer  (We called this service: @apacheLB@)
...
* The @tomcat@ service depends on the @mongod@ service and on the apacheLB service; hence the @tomcat@ service will not start until the @mongod@ service and the apacheLB service have successfully started.
...
h3. The apacheLB Recipe

The apacheLB Recipe does the following:
* Installs apache on the VM
* Configures the apache load balancer 
* Starts the apacheLB process
* Detects whether the server started successfully by using the PortLivnessDetector plugin to check port 8080
* Monitors the apacheLB liveness by using the "process locator":/guide/developing/lifecycle_events#locator.

The apacheLB service file is @<cloudify root>/recipes/apps/petclinic/apacheLB/apacheLB-service.groovy@.
...
* Detects whether the server started successfully
...
Once the application installation has successfully completed, you can access the PetClinic application by browsing to "http://YOUR_APPLICATION_IP_ON_EC2:8080/petclinic":http://YOUR_APPLICATION_IP_ON_EC2:8080/petclinic.
