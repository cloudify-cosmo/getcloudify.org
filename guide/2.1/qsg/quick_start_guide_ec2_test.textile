...
<img src="/guide/2.1/images/setup/pet_clinic_anatomy3.JPG" />
...
If you view any of the Groovy scripts in the tomcat recipe folder, and you will notice that they are essentially regular groovy scripts in which you can write any Groovy or Java code. The only difference is the use of the Cloudify API ("ServiceContext API":../developing/the_service_context_api). This API exposes the service runtime details to the script, so that a service can locate and use the details of another service within the same application. For example, in our PetClinic application we use the API to expose the port of the @mongod@ service instance to the all the other services in the PetClinic application.
...
<img src="/guide/2.1/images/setup/petClinic_in_webui_screenShot2.jpg" width="630" height="300"/>
