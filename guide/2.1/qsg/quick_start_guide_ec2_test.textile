---
layout: default
title: Getting Started with Cloudify on EC2
cat: Getting Started
publish: false
abstract: A quick start guide to help you get started with Cloudify on EC2. Start here if you are new to Cloudify and you want to install your application on EC2
pageord: 200
--- 

h1. Introduction

The goal of this quick start guide is to familiarize you with Cloudify in a few simple steps. For this purpose, we will use the PetClinic application (an example application that is included in the Cloudify distribution file) to demonstrate the steps required to deploy an application on an Amazon EC2 cloud. For a quick start guide to using Cloudify on a local cloud, see "Getting Started with Cloudify":/guide/qsg/quick_start_guide.
...
h4(warn). When you install MongoDB on Windows 7, you must run MongoDB with administrative credentials. For more information, see the "Installing Windows 7" section of the "(newwindow)Windows Service":http://www.mongodb.org/display/DOCS/Windows+Service page.
...
Before you install the Cloudify shell, make sure that your environment meets all the minimum requirements. For more information, see <span class="inlineExpandBtn" title="/guide/setup/cloudify_prerequisites">Cloudify Prerequisites</span>.
...
For more information, see <span class="inlineExpandBtn" title="/guide/setup/installing_the_cloudify_client">Installing the Cloudify Shell</span>.
...
For a more information about how to configure the EC2 cloud driver, see <span class="inlineExpandBtn" title="/guide/setup/configuring_ec2">Configure EC2</span>.
...
h4(info). To obtain a list of all the available commands, type @<tab>@, or to view help for a specific command, type @[cmd] --help@. Note that the Cloudify shell automatically connects to the REST service such that you do not have to run the *connect* command separately. For more information about Cloudify commands, see "(newwindow)Command Line Reference":/guide/recipe/command_line_reference.
...
For more information about Bootstrapping on EC2, see <span class="inlineExpandBtn" title="/guide/bootstrapping/bootstrapping_ec2">Boostrap EC2</span>.
...
<br />
The files required to run the specified services can be found in their corresponding folders.
...
For more information about recipes, see <span class="inlineExpandBtn" title="/guide/developing/recipes_overview">Anatomy of a Recipe</span>.
For more information about extending services, see <span class="inlineExpandBtn" title="/guide/developing/extending_recipes">Extending Service Recipes</span>.
...
For more information, see <span class="inlineExpandBtn" title="/guide/developing/application_recipe">The Application Recipe</span>.

h3. The Service Level Recipe

Every service has its own mandatory service recipe based on a <span class="inlineExpandBtn" title="/guide/developing/recipes_overview">recipe structure</span> that is common to all of them.
...
Cloudify is distributed with <span class="inlineExpandBtn" title="/guide/plugins_and_probes/probes">Scripted Probes</span> that you can configure as required. However, you can also develop your own <span class="inlineExpandBtn" title="/guide/plugins_and_probes/develop_your_plugin">Plugin Probes</span> and add them to the recipe in this folder. For example, in the PetClinic application we used our Cloudify Mongo Plugin Probe "(newwindow)cloudify-mongo-plugin.jar":http://www.gigaspaces.com/wiki/download/attachments/55935974/CloudifyMongoPlugins_sources2_0.zip to retrieve details from the @mongod@ service, and to detect port liveness.

For more information, see <span class="inlineExpandBtn" title="/guide/developing/service_recipe">The Service Recipe</span>.

h3. The Lifecycle Scripts

If you view any of the Groovy scripts in the tomcat recipe folder, and you will notice that they are essentially regular groovy scripts in which you can write any Groovy or Java code. The only difference is the use of the Cloudify API (<span class="inlineExpandBtn" title="/guide/developing/the_service_context_api">ServiceContext API</span>). This API exposes the service runtime details to the script, so that a service can locate and use the details of another service within the same application. For example, in our PetClinic application we use the API to expose the port of the @mongod@ service instance to the all the other services in the PetClinic application.

For more information, see the following topics:
* <span class="expandBtn" title="/guide/developing/lifecycle_events">Lifecycle Events</span>
* <span class="expandBtn" title="/guide/developing/custom_commands">Custom Commands</span>
* <span class="expandBtn" title="/guide/developing/plugins">Probes</span>
* <span class="expandBtn" title="/guide/developing/configuring_ui_section">Configuring the Web Management Console</span>
...
To monitor the application,  open a web browser, and then browse to the <span class="inlineExpandBtn" title="/guide/monitoring/the_web_management_console">Web Management Console</span>. Once the page is loaded, simply login anonymously without specifying a username or password.

h4(info). You can find the exact URL displayed in the Cloudify shell when bootstrapping the local cloud. The default URL is @http://WEB_UI_SERVICE:8099@.  
...
For more information, see <span class="inlineExpandBtn" title="/guide/developing/scaling_rules">Scaling Rules</span>.
