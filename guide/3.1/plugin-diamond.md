...
{%summary%}The Diamond plugin is used to install & configure a [Diamond](https://github.com/BrightcoveOS/Diamond) monitoring agent (version 3.5) on hosts{%endsummary%}

Michael, Maybe a short overview?
...
Config values are left with their default values unless explicitly overriden.
{%endnote%}
...
Diamond's wide range of collectors, handlers and extensibility psossibilities comes with a price - It's not always promised that you'll have all the required dependencies built into your instance.
...
Since `pymongo` is not a part of the Python standard library, this will fail unless you will install it separately. See the [nodecellar example](https://github.com/cloudify-cosmo/cloudify-nodecellar-example) for more information.
{%endnote%}
