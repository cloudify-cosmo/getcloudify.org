...
* "Windows Azure":http://www.microsoft.com/windowsazure/free-trial/ account
* "Silverlight(Download Silverlight)":http://www.microsoft.com/getsilverlight/-enabled web browser (required by Azure Management Portal)
...
h1. Understanding Windows Azure Security Credentials

Understanding Windows Azure security credentials is necessary to successfully manage a Java application running on Azure.
...
The RDP PFX certificate contains both a private and public key, and is attached to an Azure deployment. After the PFX certificate is generated on your local machine, you must upload it to a specific deployment. The PFX file is password protected for uploading the certificate to Azure (this is *NOT* your Windows password). _The certificate in the PFX file is used to encrypt and decrypt the Windows passwords used to log into your Azure VMs_.

h4(info). Do not confuse the password that protects the PFX file, with the Windows password that is encrypted using the certificate saved in the PFX file and that you use to log in to your Azure VMs (role instances) via Remote Desktop.
...
# "Configuring the Azure properties file":#updateSubscriptionId
# "Creating Remote Desktop certificates":#createRDCertificate
# "Configuring Management certificates":#createManagementCertificate
# "Updating the Blob Storage details":#updateBlobStorage

h3(#updateSubscriptionId). Configuring the Azure properties file
...
h4(info). In the azure.properties file included in the Cloudify distribution file, the *cspackFolder* property defines the path to the Windows Azure SDK. If the path is not correct for your installation, then specify the correct path.

h3(#createRDCertificate). Creating Remote Desktop certificates (Optional)

This section describes how to create the certificats for remote access to your VMs. For this example, we will use the password *123456* to encrypt the PFX files.
...
h4(info). Choose a password with at least 10 characters, containing capital letters, lower case letters, and numbers.
...
h3(createManagementCertificate). Configuring Management (API) certificates (Optional)
...
# Create the management API certificate using the following steps:
## Create the certificate--At the Windows command prompt, type: *encutil* -create -alias @azure_api@ -pwd @123456@ -pfx @azure_api.pfx@ -cert @azure_api.cer@
## Save the certificate thumbprint--At the Windows command prompt, type: *encutil* -thumbprint -cert @azure_api.cer@
...
## Install the management certificate on the local machines certificate store.
...
h4(info). Make sure when you browse for the file that the browse dialog box filter is PFX and not CER.

## Delete the PFX file.
h4(info). The certificate is saved in the certificate store and can be exported if required for a different computer.
...
h4(info). If necessary, create a new atorage account.
