...
h4(noteYellow).  Note the order of calls:
...
h4(noteYellow). Note that the plugin implementation class in this sample is _org.cloudifysource.usm.jmx.JmxMonitor_, and it receives an initial configuration of Key/Value pairs which map an attribute's display name to the JMX ObjectName notation of the relevant attribute, as well as the JMX port used to access the JMX Agent.
