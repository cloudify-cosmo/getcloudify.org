...
<h4 class="noteYellow">Please note the difference between a service and a service instance. A service can be defined as a cluster which is composed of at least one service instance.</h4>
...
To be on the safe side, specify a timeout of at least 50 minutes as described above, which means that the shell will wait for 50 *minutes* for the application to install.
 ...
 <h4 class="noteYellow"> Note: Currently the Cloudify shell does not support backslashes so your path separator has to be forward slash even if you're running on Windows.</h4>
