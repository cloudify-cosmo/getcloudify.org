...
* In the "(newwindow)EC2 Console(Note that this link leads to US east key pairs menu)":https://console.aws.amazon.com/ec2/home?region=us-east-1#s=KeyPairs create a key pair and download the private key (.pem file).
...
2. Edit the @ec2-cloud.groovy@ file located at @<Cloudify Root>/tools/cli/plugins/esc/ec2/@ and set the following:
* In the user section, update the following attributes' values: 
 ** @user@ - Insert your EC2 Access Key ID.
You can get an active Access Key ID from the Access Credentials section in "(newwindow)EC2 Console(Link to the Access Credentials section in EC2 Console)":https://aws-portal.amazon.com/gp/aws/developer/account/index.html?action=access-key as shown here:
<img src="/guide/images/ec2/EC2_ACCESS_KEY_AND_SECRET_KEY3.jpg" />
 ** @apiKey@ - Insert your EC2 Secret Access Key.
You can get a Secret Access Key from the "(newwindow)Access Credentials section in EC2 Console(Link to the Access Credentials section in EC2 Console)":https://aws-portal.amazon.com/gp/aws/developer/account/index.html?action=access-key as shown above.
<h4 class="warn">Note that the secret key must correspond to the above Access Key ID (that you set in the *user* attribute).</h4>
** @keyFile2 - Insert the name of the private key file that you created (i.e.: @myKeyPair-name.pem@ in our example).
* In the templates section, update *keyPair* under *options* ("jclouds":http://www.jclouds.org/documentation) .    
    ** You need to insert the name of the key pair that you created (whose content is in the .pem file in *keyFile* attribute in the user section).

<h4 class="info">Click "here":/guide/bootstrapping/bootstrapping_ec2 for a thorough walk-through of how to configure the EC2 cloud driver.</h4>
...
* Local agent: A component that runs on any Cloudify managed VM, and is responsbible starting application services, gathering monitoring metrics and monitoring their availability.
...
* The Web Management Console Service: provides a web based, graphical UI to the Cloudify environment and allows you to easily monitor your applications.
