...
* The @tomcat@ service depends on the @mongod@ service; hence the @tomcat@ service will not start until the @mongod@ service is successfully started.
...
The mongo-config service file is @<cloudify root>/examples/petclinic/mongoConfig/mongoConfig-service.groovy. 
...
The mongos service file is @<cloudify root>/examples/petclinic/mongos/mongos-service.groovy.
...
Once the application installation has successfully completed, you can access the PetClinic application by browsing to "http://YOUR_APPLICATION_IP_ON_EC2:8080/petclinic-mongo":http://localhost:8080/petclinic-mongo.
...
h4(info). You can find the exact URL displayed in the Cloudify shell when bootstrapping the local cloud. The default URL is "http://WEB_UI_SERVICE:8099":http://localhost:8099.  
...
To shut down the local cloud installation, at the Cloudify shell prompt, type: *teardown-cloud* @ec2@
