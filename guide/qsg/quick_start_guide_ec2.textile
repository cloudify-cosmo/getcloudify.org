...
h4(info). Make a note of the name as this information will be required for configuring the Cloudify cloud driver. For this example, we will use *myKeyPair-name* that creates the corresponding *myKeyPair-name.pem* key pair file.
...
h4(info). Make sure the file is saved under @<Cloudify Root>/tools/cli/plugins/esc/ec2/upload@.

<img src="/guide/images/ec2/EC2_CREATE_KEY_PAIR_SUCCESS_AND_SAVE3.jpg" />
h4(info). The private key enables Cloudify cloud driver to create SSH connections to newly created instances on EC2.
...
h4(info). The Secret Access Key must correspond to the specified Access Key ID.
