...
The goal of this quick start guide is get you familiar with Cloudify with a few simple steps.For this purpose, e will use one of our built-in examples (the "PetClinic" application, a port of the classis Spring PetClinic sample application which uses [MongoDB|http://www.mongodb.org] as the database instead of MySql).
This guide will teach you how to: 
* Use an example recipe to deploy an application on a [localcloud|http://www.gigaspaces.com/wiki/display/CLOUD/Bootstrapping|Bootstrapping] using Cloudify, starting from how to install Cloudify  and ending in monitoring the deployed application.
* Use the Cloudify Command line tool.
* Monitor your deployed application, using Cloudify's [Web Management Console|The Web Management Console].

h1. About the Pet Clinic Application
...
# The Data Tier - Composed of [a sharded MongoDB cluster|http://www.mongodb.org/display/DOCS/Sharding+Introduction|Sharded MongoDB], which is composed of the following components:
## [Configuration Server|http://www.mongodb.org/display/DOCS/Configuring+Sharding#ConfiguringSharding-ConfigServers|MongoDB Configuration Server] : Stores the cluster's metadata, which includes basic information on each shard server and the chunks contained therein.
...
Then cd to {{<Cloudify Root>/bin}} folder and run the {{cloudify.sh(bat)}} script. 
...
{tip}{*}Tip*: You can hit tab to view the list of available commands, or type {{help}} to see all of the command with their description. If you wish to see help for a specific command simply type {{<command name> \--help}}{tip}
...
!GRA:Images2^pet_clinic_anatomy3.jpg!
...
* A service file ({{<service name>-service.groovy}})  which can contain some or all the following sections:
...
The only unique Cloudify API that is being used is the [ServiceContext API|http://www.gigaspaces.com/wiki/display/CLOUD/The+Service+Context+API|ServiceContext API]. This API exposes the service runtime details so that one service can locate and use the details of another service within the same application.
...
// In <cloudify root>/examples/petclinic/mongos/mongos_install.groovy
...
<pre>
cloudify@default> install-application -timeout 10 <Cloudify root>/examples/petclinic
</pre>
...
{note}Currently the Cloudify shell does not support backslashes so your path separator has to be forward slash even if you're running on Windows.{note}
