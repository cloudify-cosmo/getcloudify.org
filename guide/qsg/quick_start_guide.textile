...
The only unique Cloudify API that is being used is the "ServiceContext API":guide/Developing/the_service_context_api. This API exposes the service runtime details so that one service can locate and use the details of another service within the same application.
...
To monitor the application open a web browser (the latest versions Firefox or Chrome, IE is not fully supported at this point) and log into the "Web Management Console":http://www.gigaspaces.com/wiki/display/CLOUD/The+Web+Management+Console (The shell gives you the exact URL when bootstrapping the localcloud). Usually it would be "http://localhost:8099":http://localhost:8099. You don't need to specify any username or password, just click the login button to login anonymously. 
