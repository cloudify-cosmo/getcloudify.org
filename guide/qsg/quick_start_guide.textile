...
The goal of this quick start guide is to familiarize you with Cloudify in a few simple steps. For this purpose, we will use the PetClinic application (an example application that is included in the Cloudify distribution file) to demonstrate the steps involved required to deploy an application in a local cloud. For a quick start guide to using Cloudify on EC2, see "10-Minute EC2 Quick Start Guide":guide/qsg/quick_start_guide_ec2.
...
The local cloud is a full-fledged cloud emulation environment that allows the user to run all Cloudify management and application services on a single machine.

To bootstrap the local cloud, at the Cloudify shell prompt, type: *bootstrap-localcloud*
...
h4(info). You can specify any preferred name for life cycle handlers, as long as you specify their exact names in the corresponding service file.
...
In this example, we use our powerful ServiceContext API and its k/v store mechanism, to share the @mongod@ service instance (host and port) details with the @tomcat@ service instance.
