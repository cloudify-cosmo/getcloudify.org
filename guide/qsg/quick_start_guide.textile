---
layout: default
title: 10 Minutes Quick Start Guide
cat: Getting Started
publish: true
...
# The Data Tier - Composed of and single mongod instance. 

h4(info). The "Amazon Quick Start Guide":guide/qsg/quick_start_guide_ec2 shows how to deploy the same application on Amazon EC2, and contains a sharded MongoDB configuration with multiple mongod instances
...
<h4 class="warn">When you install MongoDB on Windows 7, you need to make sure that you run as an administrator. See "Installing MongoDB on Windows 7":http://www.mongodb.org/display/DOCS/Windows+Service.
</h4>
...
Then cd to the @<Cloudify Root>/bin@ folder and run the @cloudify.sh@ or @cloudify.bat@ script. 
You should get the below Cloudify prompt:
...
(info)h4. You can hit tab to view the list of available commands, or type @help@ to see all of the command with their description. If you wish to see help for a specific command simply type @<command name> --help@.
...
The Pet Clinic application has 2 services:
# Tomcat  (We named this service: tomcat)
# MongoDB (We named this service: mongod)
...
* That the tomcat service depends on mongod service.
...
*# *A prolog section* which contains the service name, type, number of required instances and the icon to display for this service in the "Cloudify Web Management Console":/guide/monitoring/the_web_management_console
...
Here are the relevant code snippets that show how we expose the mongod service instance (host and port) to the tomcat service instance.
...
From this point, the tomcat service instance knows the host address and port of the mongod service instance.
...
To uninstall the application you should type the following: @uninstall-application petclinic@
...
* Install our extended Pet Clinic Application "on EC2":/guide/qsg/quick_start_guide_ec2.
