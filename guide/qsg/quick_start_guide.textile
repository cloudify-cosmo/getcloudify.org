...
The goal of this quick start guide is get you familiar with Cloudify with a few simple steps.For this purpose, we will use one of our built-in examples (the "PetClinic" application, a port of the classis Spring PetClinic sample application which uses "MongoDB":http://www.mongodb.org as the database instead of MySql).
This guide will teach you how to: 
*{color:blue} Use an example recipe to deploy an application on a "localcloud(Bootstrapping)":http://www.gigaspaces.com/wiki/display/CLOUD/Bootstrapping using Cloudify, starting from how to install Cloudify and ending in monitoring the deployed application.
