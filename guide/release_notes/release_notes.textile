...
|CLOUDIFY-511 | Fix all URLs to point to the new online repositories | Packaging |
|CLOUDIFY-724 | Reorder recipes folders | Packaging |
|CLOUDIFY-660 | Support automatic scaling number of instances based on thresholds | REST |
|CLOUDIFY-523 | Allow user to download Cloudify sources and build a full Cloudify zip locally | Packaging |
|CLOUDIFY-747 | Change the mongo pet-clinic applications to inherit the service implementation | CLI |
|CLOUDIFY-767 | service instance installation timeout is too short. | REST |
|CLOUDIFY-753 | mongos poststart groovy files throws null pointer if mongod is not installed | CLI |
|CLOUDIFY-695 | Add a lightweight version of Pet-Clinic | Packaging |
