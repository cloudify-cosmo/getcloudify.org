...
|CLOUDIFY-473 | Developers can now use a single jclouds instance for all cloud driver instances.
A jclouds context (instance) is fairly heavy (&gt; 7MB on EC2) and takes a while to load.
This will save a significant amount of time when installing new services. | Cloud Driver |
...
|CLOUDIFY-540 | Second bootstrap-localcloud fails to start agent | CLI |
|CLOUDIFY-550 | Fix RuntimeException being hidden (caught) in the certain situations | REST |
|CLOUDIFY-556 | Add handling for exceptions originated in the adminAPI getters. | REST |
...
|_. ID |_. Description |_. Components |
|CLOUDIFY-549 | JClouds NullPointerException when parsing images | CLI Cloud Driver |
...
|CLOUDIFY-552 | The Installation of a recipe doesn't stop | CLI USM |
|CLOUDIFY-559 | Service installation will fail in the Start event if the .out file is in use(locked). | USM |
|CLOUDIFY-560 | bootstrap-localcloud should set JAVA_HOME | CLI |
|CLOUDIFY-564 | Can't find resource for bundle java.util.PropertyResourceBundle, key failed_to_invoke_instance | CLI |
|CLOUDIFY-566 | WebUI indicates PU is good and then in progress while installing a service that has deployment on another service | USM |
...
|CLOUDIFY-595 | teardown-cloud wrong description | CLI |
