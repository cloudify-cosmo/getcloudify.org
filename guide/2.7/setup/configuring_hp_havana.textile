...
If you need to use/set additional "(newwindow)jclouds":http://www.jclouds.org/documentation options, you can set it in @hp-havana-cloud.groovy@ which is located in the @<cloudifyRoot>/clouds/hp-havana/@ folder.
In that case, you'll need to set the options section of the relevant compute template.

You need to set the following attributes in @hp-havana-cloud.groovy@, In cloudCompute -> templates -> SMALL_LINUX : computeTemplate -> options
...
*externalRouterName* - Specify the router that the vms should use. 
 ...
 If your tenant name is mytenant, then the router that Cloudify creates for you, is called *"mytenant-router"* and you should have the following in the cloud driver (@hp-havana-cloud.groovy@): 
...
*securityGroups* - You need to create and specify the security groups that you want the vm to use.
 So, if your security group is named "myGroup", then you should have the following in the cloud driver (@hp-havana-cloud.groovy@):  *"securityGroups" : ["default"] as String[]*
