...
<br/>
Also can get the image, hardware and network IDs by the following options:
# copy the ID field from the web console
# using jcloud CLI
## download "jcloud CLI"::http://search.maven.org/remotecontent?filepath=org/jclouds/cli/jclouds-cli/1.6.0/jclouds-cli-1.6.0.zip and extract it any where om your disk
## navigate to the extracted folder @/bin@ and run @jcloud-cli@ command
## install cloudstack feature @features:install jclouds-api-cloudstack@
## get image list @jclouds:image-list --api cloudstack --credential <secretKey> --identity <apiKey> endpoint http://<server address>/client/api@
## get hardware list @jclouds:hardware-list --api cloudstack --credential <secretKey> --identity <apiKey> endpoint http://<server address>/client/api@
<br/>
