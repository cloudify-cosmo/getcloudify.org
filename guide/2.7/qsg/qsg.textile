...
h2(#step1). Bootstrap Local Cloud

* *Start the cloudify shell*
Invoke the following command :<pre class='prettyprint'>gigaspaces-cloudify-2.7/bin $ ./cloudify.sh</pre>

* *Bootstrap the local cloud* 
This will install Cloudify management,agent and UI on your machine.
Invoke the following command : <pre class='prettyprint'>cloudify@default>bootstrap-localcloud</pre>
...
...Local-cloud started successfully. Use the teardown-localcloud command to shutdown all processes.</span>
...
h2(#step2). Step 2 - Deploy Application
...
Invoke the following command: <pre class='prettyprint'>cloudify@default>install-application helloworld</pre>
...
Once the application installation has successfully completed, you can access the *helloworld* at the following URL:  "(newwindow)http://127.0.0.1:8080/helloworld":http://127.0.0.1:8080/helloworld.

h2(#step4). Step 4 - Uninstall the Application

To uninstall the application, invoke the following command: <pre class='prettyprint'>cloudify@default>uninstall-application helloworld</pre>

h2(#step5). Step 5 - Teardown the Local Cloud

To teardown the local cloud installation, invoke the following command: <pre class='prettyprint'>cloudify@default>teardown-localcloud</pre>
