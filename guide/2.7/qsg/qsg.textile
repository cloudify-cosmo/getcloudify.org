...
<pre class='prettyprint'>gigaspaces-cloudify-2.7/bin $ ./cloudify.sh   </pre>
...
To uninstall the application, invoke the following command: 

<pre class='prettyprint'>cloudify@default> uninstall-application helloworld   </pre>

h2. Teardown the Local Cloud

To teardown the local cloud installation, invoke the following command: 

<span class='prettyprint'>cloudify@default> teardown-localcloud   </span>

h2. Install a Full Blown Application

To deploy a full blown application, invoke the following command: 

<pre class='prettyprint'>cloudify@default> install-application petclinic-simple   </pre>

This will install the PetClinic application, which is a port of the Spring PetClinic application to Grails and MongoDB.
Once the application installation has successfully completed, you can access the *petclinic* at the following URL:  "(newwindow)http://127.0.0.1:8080/petclinic":http://127.0.0.1:8080/petclinic.
