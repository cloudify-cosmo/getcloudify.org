...
Grizzly and Havana cloud drivers support the creation of service-specific security groups.
This is useful, for example, when the installed service requires open communication with public clients, e.g. a web server service would like to open port 8080 for incoming requests.
This is achieved by defining a security group rule in the *service* groovy file:

<script src="https://gist.github.com/noakup/8541546.js"></script>

Here an access rule (security group rule) is created to allow incoming requests from any client, through port 8080.
The template name simply specifies the relevant network template defined in the cloud groovy file.
