...
Cloudify 2.7 provides network abstraction to allow applications to define and create their network topology as part of the application deployment.
...
* Application Services Security Groups - The recipe DSL allows the user to define access rules per service. Cloudify Cloud Driver uses these rules to create security groups and bind the service VMs to these groups
...
Note that like any network, the management network (if defined) must have at-least one subnet. 
