...
As a result, other than meeting the "minimum requirements for images":/guide/setup/post_installation_configuration#images, special machine images are not required. Any *nix or Windows image can be used, whether virtual or physical. Additionally, provisioning does not necessarily have to use a real "cloud", you can use an arbitrary set of non-virtualized hosts to create "BYON":/guide/setup/configuring_byon or "local cloud":/guide/bootstrapping/bootstrapping_localcloud environments.
...
The cloud driver is a Java POJO that uses a configuration file. When defining a cloud's configuration file, you must define its class name in its cloud driver configuration file (@<cloudifyInstallationFolder>/tools/cli/plugins/esc/<cloudName>/<cloudName>-cloud.groovy). Cloudify will instantiate this class (using the @no-arg@ constructor) as required, and delegate scale out/in requests to it.
...
h2. Developing & Using Custom Cloud Drivers

Since the cloud driver is an open interface, users can implement their own cloud driver to support specific private cloud environments, or to create a more granular configuration of public cloud environments.
...
* *Loading custom class files*--The cloud driver implementation classes must be available to both the client that bootstraps the cloud, and to the Cloudify ESM management components, which are responsible for scaling services. Currently, it is the cloud driver developer's responsibility to make sure that the classes are on the classpath of the required components, for example, by adding the jar file containing your classes to the Cloudify distribution under @<cloudifyInstallationFolder>/lib/required@. These jar files are automatically added to the classpath of all components, and by adding them to the distribution they will always be available when provisioning new machines. Typically, the distribution file is downloaded from a cloud'd local network file store (e.g. Amazon S3 in the case of Amazon).
* *Loading custom startup scripts*--Once the bootstrap or scale-out process successfully provisions a new machine, a startup script is executed on the remote machine over SSH. The script is responsible for downloading and installing Cloudify, and then starting the Cloudify agent. In some implementations, you may need to tweak this process to fit the requirements of specific environments (e.g., system proxies, mount points, and paths). You can use the default startup script (@<cloudifyInstallationFolder>/tools/cli/plugins/esc/ec2/upload/bootstrap-management.sh@), which works with the built-in cloud driver, as the basis for the startup script for your own cloud driver. Simply place the modified script in the upload directory corresponding to your cloud of choice (e.g. For EC2, @<cloudifyInstallationFolder>/tools/cli/plugins/esc/ec2/upload/) and the script will be automatically uploaded to every new machine.

For more information, see "Developing Custom Cloud Drivers":/guide/clouddrivers/developing_custom_clouddriver.