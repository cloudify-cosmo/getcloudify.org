...
h4(noteYellow). Note: On clouds other than Azure, Cloudify only supports *-nix hosts that have ssh enabled. Generic windows support is in the workings.
...
h4(noteYellow). Note that in such a case, Cloudify will re-issue the request for the new machine.
