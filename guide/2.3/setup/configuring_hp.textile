...
# Copy the key and save it as a @.pem@ private key file (e.g. @myKeyPair-name.pem@) in the @<cloudifyRoot>/tools/cli/plugins/esc/hp/upload@ folder.
...
Edit the HP cloud driver configuration file (@hp-cloud.groovy@) located in the @<cloudifyRoot>/tools/cli/plugins/esc/hp/@ folder, and modify the following attributes:
...
* In the *user* section:
** *user*--Specify your credentials *Tenant Name:Access Key ID*. The Access Key ID is *NOT* the tenant ID and the Tenant Name is usually the account's email address.
e.g : joe@friends.com:42981781133893. Refer to the "(newwindow)HP console":https://console.hpcloud.com/account/api_keys for these details.
...
<h4 class="impt">To learn how to get the credentials and all the other details and how to use them, <a href="http://www.youtube.com/watch?v=JsRqbuGUmVc" rel="JsRqbuGUmVc" class="video fancybox-media">click here</a>.</h4>
