...
h4(impt). Roles are not hierarchical - they do not fully contain one another. For example, a Cloud Admin can tear down the cloud, but cannot deploy applications. If you wish to allow a user to perform both actions - grant the user both roles.
...
<a class="lightbox" href="../../images/setup/rolebased_sec_2_3_0.jpg"><img src="../../images/setup/rolebased_sec_2_3_0_thumb.jpg" alt="pic is missing" /></a>
...
Only some users are authorized to access each resource, hence the name - "resource-based authorization". Group-based authorization is performed only after the user has been authenticated and verified as holding a role that grants the access required for the attempted action (e.g. the user has the role "App Manager" which is required when attempting to delete an application). At this stage the user's groups are retrieved from the user store according to the configuration, and compared to the groups associated with the target resource. Only if the user is a member of at least one of the resource's groups, this resource is accessible to the user.
...
h2(#confsec). Configuring & Bootstrapping a Secured Cloudify Manager
...
To protect the data communication between different clients and the server, Cloudify uses HTTPS with SSL encryption.
...
<a class="lightbox" href="../../images/setup/securityxml.jpg"><img src="../../images/setup/securityxml_thumb.jpg" alt="pic is missing" /></a>
...
<a class="lightbox" href="../../images/setup/authgroupxml.jpg"><img src="../../images/setup/authgroupxml_thumb.jpg" alt="pic is missing" /></a>
