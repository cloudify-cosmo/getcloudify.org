...
* If the path is to a groovy file, then the file should be named *-tempalte.groovy and it should be the only one with that suffix in its directory. 
...
All the template groovy files that have prefix other than the template's name, will be renamed to &lt;template name&gt;-template.groovy.
