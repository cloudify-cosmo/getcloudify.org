...
** Currently <code>org.openspaces.usm.jmx.JmxDetails</code> using JMX Mbeans attributes, is the only built-in plugin
* *Monitoring Plugin* - used to get custom metrics sampled once every X seconds and reported to Admin API and Web Console
** Currently, <code>org.openspaces.usm.jmx.JmxMonitor</code>, using JMX Mbeans attributes, is the only built-in plugin.
...
* Add a custom plugin. Custom plugins and their dependencies should be located (as jars) under <code>usmlib</code> sub-folder
