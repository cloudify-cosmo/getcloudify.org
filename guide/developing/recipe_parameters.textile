...
The properties file must be saved in the same folder as the service descriptor file, and we recommend using the following naming convention: @<myRecipe>-service.properties@ (e.g. jboss-service.properties). The keys are defined using a simple @Key=Value@ notation (e.g. @myKey=myValue@) and can be used in the DSL closures as string parameters. They can also be used in other strings using the format @${Key}@ (e.g. @${myKey}@).

The following shows an example properties file and a section of its corresponding service descriptor file that uses the parameters.
