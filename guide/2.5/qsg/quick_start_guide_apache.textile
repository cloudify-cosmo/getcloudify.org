...
* Use an example recipe to deploy an application on a "Local Cloud":../bootstrapping/bootstrapping_localcloud.html using Cloudify, from the installation of Cloudify iteself, deployment of your applocation and finally monitoring you deployed application using Cloudify's management tools.html
...
Cloudify comes with "built-in plugin implementations":../plugins_and_probes/develop_your_plugin.html that you can configure as needed, but you can also develop your own monitoring plugins and pack them with the recipe (in jar files under usmlib sub-folder within each service folder).html
