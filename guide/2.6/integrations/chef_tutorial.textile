...
<h4 class="info">travel-chef application recipe is comprised of four services: <ul>
...
If the mysql recipe needs to add any additional implementations, such as monitors. scaling rules, etc', the place to put them is right after the "extend" statement in the chef-server service file (mysql-service.groovy). 
