...
h2(#prepost). Pre-Bootstrap and Post-Bootstrap scripts
...
h2(#ipfilter). Configuring Private and Public IPs filters
...
In other cases, users may decide to configure it themselves and not leave this descision to Cloudify. 
Users can configure a range of private IPs and/or a range public IPs, according to a CIDR or regular expression filter.
With this feature, users can explicitly define which IP addressess should be considered public and which should be private, by specifying a CIDR filter or a regular expression filter.
Only an IP address that matches such a filter (if defined) will be used.
Users can set one filter for public at most and one for private at most.
...
<br/>
For public IP addresses, users can set either org.cloudifysource.default-cloud-driver.public-ip-cidr or org.cloudifysource.default-cloud-driver.public-ip-regex for public IPs, but not both.
And the same goes to private : 
Users can set org.cloudifysource.default-cloud-driver.private-ip-cidr or org.cloudifysource.default-cloud-driver.private-ip-regex for private IPs, but not both.

The following snippet shows an example of usage :
<script src="https://gist.github.com/tamirko/5804221.js"></script>

The logic is as follows :
# Scan jclouds public addresses, look for entry matching public cidr filter.
# If not found, scan jclouds public addresses, look for entry matching public regex filter.
# If not found, scan jclouds private addresses, look for entry matching public cidr filter.
# If not found, scan jclouds private addresses, look for entry matching public regex cidr filter.
# If not found, return first address in jclouds public addresses.
# If jclouds public addresses is empty, public address is null.

h4(impt). If in step 3, Cloudify finds a jclouds private address that matches a public CIDR filter, then this address will be considered a public address by Cloudify.

The logic for a private address is similar. - Just first scan private addresses and match to a private cidr/regex filter.

The implementation can be found available in the  "(newwindow)getAddress":https://github.com/CloudifySource/cloudify/blob/master/esc/src/main/java/org/cloudifysource/esc/driver/provisioning/jclouds/CloudAddressResolver.java method of CloudAddressResolver.
