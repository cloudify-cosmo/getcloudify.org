...
* Windows Azure SDK - *August 2011* v1.4 <a href="http://www.microsoft.com/windowsazure/sdk/">Azure SDK</a>. After the installation is complete select to install also the "Windows Azure Tools for Microsoft Visual Studio 2010"
...
* The Cloudify installation with the Cloudify Interactive Shell <a href="http://gigaspaces.com/cloudify">Download Cloudify</a>
...
!CLOUD:Cloudify Documentation Home^certificates1.png!
...
p.
# Install Cloudify see (link to install here)


h3.Update Subscription ID in azure.properties

p.Login to http://windows.azure.com
...
<img src="images/config3b.png"/>
# Paste it into  <Cloudify Home>\tools\cli\plugins\azure\azure.properties as the value of "subscriptionId"


h3. Create Remote Desktop Certificates 

p.In this tutorial we will use the password 123456 in order to encrypt pfx files.
...
{code}cd <Cloudify Home>\tools\cli\plugins\azure
encutil -create -alias azure_rdp -pwd 123456 -pfx azure_rdp.pfx -cert azure_rdp.cer{code}
...
{code}encutil -encrypt -text MyPassword007 -cert azure_rdp.cer > password.txt
notepad password.txt{code}
Copy the encrypted password and paste it into c:\cloudify\tools\cli\plugins\azure\azure.properties file as the value of "rdpLoginEncryptedPassword"
...
p.This step is only required if you wish to change the number of role instances from the interactive shell (scale out)
# Create the management API certificate:
{code}encutil -create -alias azure_api -pwd 123456 -pfx azure_api.pfx -cert azure_api.cer{code}
# Save the certificate thumbprint, for later use.
{code}encutil -thumbprint -cert azure_api.cer{code}
# Copy the thumbprint and paste it into <Cloudify Home>\tools\cli\plugins\azure\azure.properties as the value of "certificateThumbprint"(required for scaling out)
...
p.Log in with your Windows Live credentials to the Azure Management Portal at [http://windows.azure.com] 

# From the bottom-left menu select "Hosted Services"
!CLOUD:Cloudify Documentation Home^config7.png!
# From the top-left menu select "Management Certificates"
!CLOUD:Cloudify Documentation Home^config8.png!
# Click the "Add Certificate" icon
(top left)
!CLOUD:Cloudify Documentation Home^config9.png!
# Browse to azure_api.cer file
!CLOUD:Cloudify Documentation Home^config10.png!
...
# Copy the storage account name and paste it to <Cloudify Home>\tools\cli\plugins\azure\azure.properties sotrageAccount value.
# Click the "View Access Key" button and copy the Primary Access Key to the azure.properties storageAccessKey value
!CLOUD:Cloudify Documentation Home^blobstorage_accesskeys.png!