...
This tutorial will help you get started with integrating chef recipes into your cloudify applications. We will assume that you are familiar with the basics of the cloudify platform (if not, we suggest "the cloudify quick start guide":../qsg/quick_start_guide) and the basics of the opscode chef platform (if not, we suggest "the chef fast start guide":http://wiki.opscode.com/display/chef/Fast+Start+Guide).
