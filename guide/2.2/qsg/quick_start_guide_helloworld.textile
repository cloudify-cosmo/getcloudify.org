...
The goal of this quick start guide is to familiarize you with Cloudify in a few simple steps. For this purpose, we will use a Hello World application (an example application that is located in our "(newwindow)GitHub repository":https://github.com/CloudifySource/cloudify-recipes/tree/master/apps/helloworld and will soon be included in the Cloudify distribution file) to demonstrate the steps required to deploy an application on a local cloud.
...
Now that the cloud is ready, let's deploy the application. At the Cloudify shell prompt, type: *install-application* @-timeout 10 <cloudify root>/recipes/apps/helloworld@
