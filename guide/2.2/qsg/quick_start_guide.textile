...
The goal of this guide is to familiarize you with Cloudify in a few simple steps. For this purpose, we will use the PetClinic application (an example application that is included in the Cloudify distribution file) to demonstrate the steps required to deploy an application on a local cloud. For a guide to deploying Cloudify on EC2, see "Deploy a Multitier App on EC2":../qsg/quick_start_guide_ec2.
