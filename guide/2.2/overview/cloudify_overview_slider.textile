...
<div>
<div id="motivation">
<p>Some of the main impetuses for moving to the cloud include:</p>
...
<img src="../../images/overview/onboard.png" width="635px" />

The following diagram shows the architecture that enables this simple process:
<img src="../../images/overview/architecture.png" width="635px" />
...
This architecture therefore enables you to write recipes describing all the components required to run your application and leaves Cloudify to handle the details of implementing the environment. The recipes are simple Groovy files that describe the execution plans for the lifecycle of the application (for installing, starting, orchestrating, and monitoring your application stack) and use cloud drivers configuration files (that describe the machines and their images for your chosen cloud) for provisioning machines in your cloud.

Using this allows you to achieve your objectives as follows:
* *No Code Change* -- This is achieved using application recipes that describe the services and their dependencies.
<img src="../../images/overview/universal-service-agents.png" title="Built-In Recipes" width="610px"/>
...
<img src="../../images/overview/no-lockin.png" title="Built-In Recipes" width="610px"/>
...
<img src="../../images/overview/full-control.png" title="Built-In Recipes" width="610px"/>
...
<img src="../../images/overview/Built-in-recipes.png" title="Built-In Recipes" width="635px"/>
</div>
<div id="features" style="padding-left:0;padding-right:0;">
...
<div id="pageSlider">
<ul>
	<li><iframe id="frmMotivation" class="sliderFrame"></iframe></li>
	<li><iframe id="frmCloudify" class="sliderFrame"></iframe></li>
	<li><iframe id="frmArchitecture" class="sliderFrame"></iframe></li>
	<li><iframe id="frmFeatures" class="sliderFrame"></iframe></li>
</ul>
</div>

<script type="text/javascript">
	$(function() {
		var slideTitle = new Array("Motivation for Cloud", "About Cloudify", "How Cloudify Works", "Cloudify Features"),
			frmWidth = "667px",
			frmHeight = "700px";
		
		$('#pageSlider').easySlider({auto:true, numeric:true, pause:5000});
		
		$('#pageSlider').css({"height":frmHeight});
		$('#pageSlider ul, #pageSlider li').css({"margin":"0", "padding":"0", "list-style":"none"});
		$('#pageSlider li').css({"height":frmHeight, "width":frmWidth, "overflow":"hidden"});
		
		$('#frmMotivation').contents().find('body').append($('#motivation'));
		$('#frmCloudify').contents().find('body').append($('#cloudify'));
		$('#frmArchitecture').contents().find('body').append($('#architecture'));
		$('#frmFeatures').contents().find('body').append($('#features'));
		
		$('.sliderFrame').css({"border":"0", "height":frmHeight, "width":frmWidth});
		$('.sliderFrame').contents().find("html").css("background", "none");
		$('.sliderFrame').contents().find("head").append('<link rel="stylesheet" href="/css/template.css" type="text/css"/>');
		
		for (var x=0; x<slideTitle.length; x++) {
			$('#controls' + (x+1) + ' a').text(slideTitle[x]);
		}
		
		$('#pageSlider').prepend($('#controls'));
	});
</script>