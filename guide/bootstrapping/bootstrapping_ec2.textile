...
<h4 class="noteYellow">Note that the secret key must correspond to the above Access Key ID (that you set in the *user* attribute).</h4>
 ...
 ** *imageId* - Insert the Amazon Machine type ID.
     <h4 class="noteYellow">Note that different EC2 regions have different AMIs for the same image configuration.*</h4>
     You can find the available types in "(newwindow)EC2 images types page":https://console.aws.amazon.com/ec2/home?region=us-east-1#s=Images as shown here:
<img src="/guide/images/ec2/ec2_images_types.jpg" />
