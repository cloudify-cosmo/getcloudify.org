...
h4(noteYellow). Note that the bootstrapping script is aimed at handling the most common installation scenario, but some clouds, especially private clouds, require more tweaking to work correctly. By editing the bootstrap-management.sh script, administrators can modify the bootstrapping process to match their needs.
