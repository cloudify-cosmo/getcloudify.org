...
<h4 class="info"><ul><li>The bootstrap-management.sh is copied (via SSH) from the client's upload folder (&lt;Cloudify-Root&gt;/tools/cli/plugins/esc/byon/upload) to the remote directory on each deployed machine.</li>

<li>SSH is also used to invoke the bootstrap-management.sh on the target machines.</li>
...
<li>When multiple machines are started, it's executed on different threads parallelly.</li>
</h4>
