...
h4(warn). When there's an insufficient number of machines for allocation, the following exception is thrown:
...
In order to associate the SMALL_LINUX template with a service, you must specify it in the service file included with the recipe, as follows:

<script src="https://gist.github.com/2337408.js?file=cassandra.groovy"></script>

h4(info). Cloudify BYON driver allocates machines from the pool randomly. Future releases will support allocating machines based on the order specified within the pool.
...
You can also customize the id using the parameter placeholder @{0}@ which Cloudify will replace with an index value (e.g. vm{0}-lab will be generate vm1-lab, vm2-lab, etc.).
...
To shut down the BYON installation, at the Cloudify shell prompt, type *teardown-cloud* @byon@.