...
You can then install any of the built-in examples. For installing the Pet Clinic Application, type: 
@install-application -timeout 10 ../../examples/petclinic@

For more information, see the "Quick Start Guide":/guide/qsg/quick_start_guide.

h1. Enabling wire logging

For development and debugging purposes, it can be usefull to view the actual messages sent to the Openstack cloud controller. To do this, modify your openstack-cloud.groovy by setting the "openstack.wireLog" entry in the custom section of the file to "true".

For example: 

<script src="https://gist.github.com/2035261.js?file=custom-wireLog.groovy"></script>

Once you enable the wire logs, they appear in the Cloudify command line interface when you run the 'bootstrap-cloud' command (and in the relevant log file in the logs directory) as well as in the log of the ESM when it starts or stops a machine for scale out/in.
